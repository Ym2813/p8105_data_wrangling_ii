...
R will go to the internet and import the entire html file that comes out
of the url
...
**Note** Every time we run the following code, r will call the internet
and try to download things, some time we need this, but other times we
don’t. So if we separate this step with the rest (separate
drug\_use\_html with drug\_use\_df, instead of piping them together), we
can read in the html file first, and as we refine the code for the df in
the separate code, we are manipulating it with a local version of the
html file instead of downloading it every time.

``` r
drug_use_html = read_html(url)
```

**table** Now R will take the html and try to extract all the tables in
it has and format it as a data frame.
...
We don’t have any tables in the web. Suppose we want the title, rating,
and how much money they made, etc. All those things we want is sorted in
the same place, for each movie, therefore, we can use the CSS selector.
Use selector gadget

Use the selector gadget to select one thing u want highlight in green,
and it will get all the other things that are using the same CSS tag. It
will return a lot of things highlighted in yellow, all of which are
using the same tag. Click on the highlighted parts that u don’t want,
make them red. Check and see if only the things u want are highlighted
in yellow. Then copy the title that selectorgadget identified, for
example, “.lister-item-header a”
...
Instead of working with something that already exist, we send a request
to a server
